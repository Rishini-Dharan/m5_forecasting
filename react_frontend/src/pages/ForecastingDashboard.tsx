...
export default function ForecastingDashboard() {
  ...
  // Real recorded sell_price for the selected series -- never an invented number.
  useEffect(() => {
      if (!formData.item_id || !formData.store_id) return;
      let cancelled = false;
      const fetchPrice = async () => {
          ...
          try {
              ...
              if (response.ok && !cancelled) {
                  const data = await response.json();
                  setFormData(prev => ({ ...prev, price: data.sell_price }));
              }
          } catch (e) {
  ...
  // Gain-based feature importance straight from the trained LightGBM boosters.
  useEffect(() => {
  ...
  const ImportanceTooltip = ({ active, payload }: any) => {
      if (active && payload && payload.length) {
          ...
          return (
              <div className="bg-gray-900 border border-gray-700 p-3 rounded-lg shadow-xl font-mono text-xs">
                  <p className="text-gray-300 mb-1">{row.feature}</p>
                  <p className="text-gray-400 m-0">{(row.share * 100).toFixed(1)}% of total gain</p>
              </div>
          );
      }
      return null;
              ...
              <div className="mb-8">
                  <div className="flex items-center gap-3 mb-4">
                      ...
                      <div>
                          ...
                          <p className="text-gray-400 text-sm">Generate 28-day unit-sales forecasts from the trained M5 LightGBM models</p>
                      </div>
                  </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                  ...
                  <div>
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                          Prefilled with the real recorded price. Change it to run a what-if.
                      </p>
                  </div>

                  {/* SNAP Day. There is no weekend toggle: the model derives the day of week
                      from the real calendar for each forecast date, so overriding it would be
                      meaningless. */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              <div className="bg-gray-900 rounded-xl border border-gray-800 p-6">
                  <div className="flex items-center justify-between">
                      <p className="text-sm font-medium text-gray-400">Total Predicted (28 days)</p>
                      <span className="w-8 h-8 rounded-lg flex items-center justify-center text-white text-sm" style={{ backgroundColor: WALMART_BLUE }}>
                          📈
                      </span>
                  </div>
                  ...
                  <div className="flex items-center gap-4">
                      ...
                      <div>
                          ...
                          <p className="text-sm text-gray-400">
                              ...
                              {formData.price > 0 && ` • Price: $${Number(formData.price).toFixed(2)}`}
                              {forecastMeta && ` • Origin ${forecastMeta.origin}${forecastMeta.scenario ? ' • what-if applied' : ''}`}
                          </p>
                      </div>
                  </div>
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
              ...
              <div className="bg-gray-900 rounded-xl border border-gray-800 p-6 h-full">
                  ...
                  <p className="text-xs text-gray-500 mb-3 -mt-2">
                      Gain-based feature importance from this store&apos;s trained LightGBM models.
                      Global to the model, not an attribution of a single forecast.
                  </p>
                  {impactData.length > 0 ? (
                      <div className="h-64">
                          <ResponsiveContainer width="100%" height="100%">
                              <BarChart data={impactData} layout="vertical" margin={{ top: 0, right: 20, left: 40, bottom: 0 }}>
                                  <XAxis type="number" hide />
                                  <YAxis dataKey="feature" type="category" width={130} tick={{ fill: '#9ca3af', fontSize: 11, fontFamily: 'monospace' }} axisLine={false} tickLine={false} />
                      ...
                      <div className="h-64 flex items-center justify-center">
                          <p className="text-gray-500 text-center">Loading model drivers...</p>
                      </div>
                  )}
              </div>
          </div>
